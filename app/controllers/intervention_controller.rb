...
def create
	...
	i = Intervention.create(author_id: current_user.id,:customer_id => to_number(params[:customer]), :building_id => to_number(params[:building]), :battery_id => to_number(b), :column_id => to_number(c), :elevator_id => to_number(e), :employee_id => params[:employee], :rapport => params[:description]) 
